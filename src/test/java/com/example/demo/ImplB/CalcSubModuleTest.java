...
@SpringBootTest()
public class CalcSubModuleTest {

    @Autowired
    @Qualifier("CalcSubModule")
    CalcModule calcModule;

    @Test
    @DisplayName("测试整数")
    public void TestInteger() {
        Assert.assertTrue(calcModule.calc(1D,2D).equals(-1D));
    }

    @Test
    @DisplayName("测试浮点")
    public void TestDouble() {
        Assert.assertTrue(calcModule.calc(1.1,2.1).equals(-1D));
    }

    @Test
    @DisplayName("测试正数")
    public void TestPositive() {
        Assert.assertTrue(calcModule.calc(101D,2D).equals(99D));
    }
